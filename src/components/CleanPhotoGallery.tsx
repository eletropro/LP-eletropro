...
import { X, ChevronLeft, ChevronRight, Maximize2, Camera, Upload, CheckCircle2, ImagePlus } from 'lucide-react';
...
export const CleanPhotoGallery: React.FC = () => {
  ...
  const getImageSrc = (item: { id: number; src: string }) => {
    return photosMap[item.id] || item.src;
  };
    ...
    <section id="galeria" className="py-12 sm:py-16 bg-slate-950 border-b border-slate-800/80">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto mb-6 space-y-2">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-yellow-400/10 border border-yellow-400/30 text-yellow-400 text-xs font-bold rounded-full">
            ...
            <span>Fotos Reais de Obras</span>
          </div>

          <h2 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight font-['Outfit',sans-serif]">
            Galeria de <span className="text-yellow-400">Instalações Executadas</span>
          </h2>
        ...
        {/* Barra de Seleção das Fotos Originais */}
        <div className="mb-8 p-4 bg-slate-900/90 border border-slate-800 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xl">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xs sm:text-sm font-bold text-white flex items-center gap-2">
                <span>Adicionar Suas Fotos Originais</span>
                {uploadedCount > 0 && (
                  <span className="inline-flex items-center gap-1 text-[11px] text-emerald-400 bg-emerald-400/10 px-2 py-0.5 rounded-md font-semibold">
                    ...
                    {uploadedCount} salvas permanentemente
                  </span>
                )}
              </div>
              <p className="text-[11px] text-slate-400 mt-0.5">
                Selecione as 10 fotos do WhatsApp de uma vez só. Elas ficam salvas de forma fixa e não somem ao atualizar.
              </p>
            </div>
          </div>
          ...
          <div className="shrink-0 w-full sm:w-auto">
            <button
              ...
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-yellow-400 hover:bg-yellow-300 text-slate-950 font-extrabold text-xs uppercase tracking-wide rounded-xl shadow-md transition-all cursor-pointer active:scale-95"
            >
              <ImagePlus className="w-4 h-4 fill-slate-950" />
              <span>Selecionar Fotos Originais</span>
            </button>
            <input
              ref={fileInputRef}
              type="file"
        ...
        {/* Grade Limpa de Fotos (Apenas exibição visual, sem poluição de texto) */}
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4">
          {GALLERY_IMAGES.map((item, idx) => {
            const currentSrc = getImageSrc(item);
            return (
              <div
                key={item.id}
                onClick={() => openLightbox(idx)}
                className="group relative aspect-[3/4] rounded-xl overflow-hidden bg-slate-900 border border-slate-800 hover:border-yellow-400/80 transition-all duration-200 cursor-pointer shadow-lg"
              >
                <img
                  src={currentSrc}
                  alt={item.alt}
                ...
                <label
                  ...
                  className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity p-1.5 bg-slate-950/80 hover:bg-yellow-400 text-slate-300 hover:text-slate-950 rounded-lg border border-slate-700 cursor-pointer z-10"
      ...
      {activeImageIndex !== null && (
        ...
        >
          ...
          >
            <img
              src={getImageSrc(GALLERY_IMAGES[activeImageIndex])}
